...
ggplot(mpg, aes(x = cyl, y = hwy)) +
  ...
  labs(
    ...
    title = "No clear relationship between numbers of cylinders and highway efficiency",
    caption = "Data from fueleconomy.gov"
  )
...
ggplot(mpg, aes(x = displ, y = hwy)) +
  ...
  annotate(
    ...
    yend = 25,
    color = "red",
    arrow = arrow(angle = 55, type = "open")
  )
...
presidential |>
  ...
  geom_point() +
  geom_segment(aes(xend = end, yend = id)) +
  geom_text(aes(x = start, y = id, label = name, vjust = -0.5, hjust = -0.5)) +
  scale_x_date(
    name = NULL,
    breaks = presidential$start,
    date_labels = "'%y"
...
ggplot(diamonds, aes(x = carat, y = price)) +
  ...
  guides(color = guide_legend(title = "Cut", override.aes = list(alpha = 0.9, size = 3)))
